...
consensus <- function(formula, data, model = "simplified", weights = NULL, 
  ...
  # sep=''))
  colnames(parameters) <- c("estimate", "robust std", "z value", 
                            "P(|z|>.)")
  ...
  out <- list(MaxLL = maxLLk, parameters = parameters, varcov1 = v1, 
              ...
              matz = matz, y = y, long = long, mui = mui, res = res, iter.detail = iter.detail, 
              call = call)
  class(out) <- "consensus"
  out
}
...
plot.consensus <- function(x, ...) {
  ...
  print.default(cor( x = abs(x$y - x$mui), y = 1/x$long,  method="spearman"), print.gap = 2, quote = FALSE, 
                right = TRUE, ...)
  cat("\n")
...
gg_qq <- function(fitted, resid, distribution = "norm", ..., line.estimate = NULL, conf = 0.95,
                  labels = names(x)){
  ...
  p1 = ggplot(df, aes(x=predicted.mean, y=residual)) +
    geom_point( colour="blue")  + geom_abline(intercept = 0, slope = 0)+ 
  ...
  gg <- gg + geom_histogram( binwidth=(range.residual[2]-range.residual[1])/10 ,colour="black", 
                             aes(y=..density.., fill=..count..))
  #gg <- gg + scale_fill_gradient("Count")
  p2 <- gg + stat_function(fun=dnorm,
                           color="red",
                           args=list(mean=mean(residual$resid), 
...
hist.resid <- function(resid){
  residual=data.frame(resid=resid)
  gg <- ggplot(residual, aes(x=resid))
  gg <- gg + geom_histogram( bins= 12,colour="black", 
                             aes(y=..density.., fill=..count..))
  gg <- gg + scale_fill_gradient("Count")
...
# Fonction pour ajuster le mod?le de r?gression angulaire acceptant des covariables
#
# Date de la derni?re mise ? jour : 27 juin 2013
# Auteur: Sophie Baillargeon
#
# Nicosia Aurelien: Ajout de l'estimateur de variance parametrique (v_0)  (01-05-2014)
#                   Variance comme dans l'article (04-02-2015) 
...
angular <- function(formula, data, model="simplified", initbeta=NULL, control=list()){
  ...
  paramname <- nomterms
  nterms <- length(nomterms)
  p <- if ("simplified" == model) nterms - 1 else nterms
  nparam <- if ("simplified" == model) p else p + 1
  ...
  # Fonction pour le calcul du crit?re max-cosine ? maximiser
  maxcos <- function(beta){ # pas en argument car ne change pas : y, x0, matx, p, model
    sinmu <- sin(if ("simplified" == model) x0 else beta[p+1]) + as.vector((matz*sin(matx))%*%beta[1:p]) # ?quation 1, 1e ?l?ment du vecteur
    cosmu <- cos(if ("simplified" == model) x0 else beta[p+1]) + as.vector((matz*cos(matx))%*%beta[1:p]) # ?quation 1, 2e ?l?ment du vecteur
  ...
  # Fonction pour la mise ? jour des beta
  betaUpdate <- function(betak, long, mui){ # pas en argument car ne change pas : y, matx, p, model
    matd <- cbind(matz*sin(matx-mui), if ("simplified" == model) NULL else cos(betak[p+1]-mui))/long # Matrice Xi de l'?quation 11
  ...
  # Valeurs initiales des param?tres beta
  if (is.null(initbeta)){ # Quelques valeurs possibles sur une grille sont essay?es
    # Objectif : essayer environ 10000 vecteurs beta (c'est trop long d'en essayer plus)
    pginit <- if(is.null(control$pginit)) 1000 else control$pginit
    pg <- round(pginit^(1/nparam))
    possbeta <- rep(list(seq(-1, 1, length.out=pg+2)[-c(1,pg+2)]), p)
    if ("complete" == model) possbeta[[p+1]] <- seq(0, 2*pi, length.out=pg+2)[-c(1,pg+2)]
    possVal <- cbind(expand.grid(possbeta),NA)
    colnames(possVal) <- c(betaname, "maxcosine")
    #for (i in 1:nrow(possVal)) {  # la boucle est un peu plus lente que le apply 
    #  possVal[i,nparam+1] <- maxcos(beta=unlist(possVal[i, 1:nparam]))$maxcos
    #}
    ...
    if (length(initbeta) != nparam) stop("for the requested model, 'initparam' must be of length ", nparam)
    betak <- initbeta
  }
  ...
  ### Calcul d'erreurs type pour les param?tres
  # Calcul de matd (matrice Xi) et res (sin(y-mui)) pour les param?tres finaux
  if (maxcosk == maxcosk1) { # Si non convergence parce que max-cosine diminue, on a d?j? tout.
    # mais si convergence ou non converge car maxiter atteint, on n'a pas
    # matd et res pour la toute derni?re mise ? jour de beta.
    maj <- betaUpdate(betak=betak, long=long, mui=mui)
  }
  matd <- maj$matd
  res <- maj$res
  ...
  dlong <- cbind(cos(matx - mui), if ("simplified" == model) NULL else -sin(betak[p+1] - mui)) # chaque ligne de cette matrice est un dlongi transpos?
  DDX <- t(dlong) %*% diag(res/long) %*% matd
  A <- (-1/nobs)*(t(matd) %*% diag(cos(y-mui)) %*% matd + DDX + t(DDX))
  Ainv <- solve(A) 
  v0 <- solve(t(matd) %*% matd)/(Akappa*kappahat)
  v1 <- (1/(nobs*(nobs-1)))*(Ainv %*% (t(matd) %*% diag(res^2) %*% matd) %*% Ainv)
  A1 <- (-1/nobs)*(t(matd) %*% diag(cos(y-mui)) %*% matd)
  A1inv <- solve(A1)
  #A1inv<-ginv(A1)
  v2 <- (1/(nobs*(nobs-1)))*(A1inv %*% (t(matd) %*% diag(res^2) %*% matd) %*% A1inv)
  Mautocorr <- t(matd[-nobs,]) %*% diag(res[-nobs]*sin(y[-1]-mui[-nobs])) %*% matd[-1,]  
  v3 <- (1/(nobs*(nobs-1)))*(Ainv %*% ((t(matd) %*% diag(res^2) %*% matd) + Mautocorr + t(Mautocorr)) %*% Ainv)
  v4 <- solve(t(matd) %*% matd)*mean(res^2)/(maxcosk^2)
  #v4 <- ginv(t(matd) %*% matd)*mean(res^2)/(maxcosk^2)
  
  ### Sortie des r?sultats
  #parameters <- cbind(betak,sqrt(diag(v0)), sqrt(diag(v1)), sqrt(diag(v2)), sqrt(diag(v3)), sqrt(diag(v4)))
  #parameters <- cbind(betak,sqrt(diag(v0)), sqrt(diag(v1)))
  ...
  #colnames(parameters) <- c("estimate", paste("stderr", 0:1, sep=""))
  rownames(parameters) <- betaname
  autocorr <- acf(res, plot = FALSE)
  
  #out <- list(MaxCosine=maxcosk, parameters=parameters,varcov0=v0, varcov1=v1, varcov2=v2, varcov3=v3, varcov4=v4, autocorr=autocorr, iter.detail=iter.detail, call=call)
  out <- list(MaxCosine=maxcosk, parameters=parameters,varcov0=v0, varcov1=v1, autocorr=autocorr, long=long,mui=mui, y = y, iter.detail=iter.detail, call=call)
...
plot.angular <- function(x, ...){
  gg_qq(fitted = x$mui, resid = x$y - x$mui)
  invisible(x)
}
