...
#' 
#' @author Aurélien Nicosia 
FilterSmoothRWM <- function(param, data, type = "angle", dist = "gamma") {
...
fitGHRW <- function(param, data, EMMax = 50, precision = 2, EMMin = 10, 
  ...
  while (norm(as.matrix(paramopnew - paramopold), type = "M") > 
    ...
    FS <- FilterSmoothRWM(param, data, type, dist)
    ...
    param <- list(pi0 = pi0, kappa = kappa, thetalin = thetalin, 
                  P = P)
    FSL <- FilterSmoothRWM(param, data, type, dist)
    LL <- FSL$LogLike
...
PlotEstim <- function(data, param, long = NULL, lat = NULL, type = "angle-dist", 
                      dist = "gamma", explo = 1) {
  ...
  if (is.null(long) || is.null(lat)){
    trajectory <- matrix(0, nrow = steps, ncol = 2)
    for (i in (1:(steps))){
      ifelse(i ==1,trajectory[i,] <-d[i]*c(cos(y[i]),sin(y[i])) ,trajectory[i,] <- trajectory[i-1,] + d[i]*c(cos(y[i]),sin(y[i])))
    }
    long <- trajectory[,1]
    lat <- trajectory[,2]
  ...
  position <- data.frame(X = long, Y = lat, proba_state_1 = E.smooth[,explo])
  head(position)
  p <- ggplot(position, aes(x=X, y=Y)) +
    geom_point(size=0.2) +xlab("Longitude") + ylab("Latitude");
  p1 = p + theme(
    axis.text.x = element_blank(),
    axis.text.y = element_blank()) +geom_point(data=head(position[,c(1,2)], 1), colour="green") +
    geom_point(data=tail(position[,c(1,2)], 1), colour="yellow") 
  p2 = p1 + geom_path(aes(colour=proba_state_1),size=0.2)+scale_color_gradient2(midpoint=0.5,  low="blue", mid="white",
                                                                                high="red",  space = "Lab")
  plot(p2)
